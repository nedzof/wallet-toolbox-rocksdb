 ...
 *
 * Test scenarios:
 *   1. Input confirmed-spent on chain (fresh getUtxoStatus → false) →
 *      basket entry transitions to spendable=false.
 *   2. Input still unspent on chain (fresh getUtxoStatus → true) →
...
describe('markStaleInputsAsSpent', () => {
  ...
  /** Mock WalletServices that returns a configured fresh getUtxoStatus result. */
  function mockServices (
    isUtxoFn: (output: any) => boolean | Promise<boolean>,
    eventBus?: { emitUtxoInvalidation: (event: { outpoints: string[] }) => void }
  ): sdk.WalletServices {
    return {
      eventBus,
      hashOutputScript: () => 'x'.repeat(64),
      getUtxoStatus: async (_output: string, _format: any, outpoint?: string, useNext?: boolean) => {
        expect(useNext).toBe(true)
        const [txid, vout] = String(outpoint ?? '').split('.')
        return {
          name: 'mock',
          status: 'success' as const,
          isUtxo: await isUtxoFn({ txid, vout: Number(vout), outpoint, useNext }),
          details: []
        }
      },
  ...
  test('marks input as spendable=false when chain confirms it is spent (regression)', async () => {
    for (const storage of storages) {
      ...
      const eventBus = { emitUtxoInvalidation: jest.fn() }

      // Chain says: NOT a UTXO (i.e. spent).
      const services = mockServices(() => false, eventBus)
      ...
      expect(eventBus.emitUtxoInvalidation).toHaveBeenCalledWith({
        outpoints: [`${seed.consumedOutpoint.txid}.${seed.consumedOutpoint.vout}`]
      })
      ...
      expect(after.cacheUpdatedAt).toBeInstanceOf(Date)
    }
  })
  ...
  test('leaves input spendable=true when chain confirms it is still a UTXO (preserves transient retry)', async () => {
    for (const storage of storages) {
      ...
      const services = mockServices(() => true, eventBus)
      ...
      expect(result.staleOutpoints).toEqual([])
      expect(eventBus.emitUtxoInvalidation).not.toHaveBeenCalled()
  ...
  test('mixed-batch: only truly-stale inputs are evicted; still-UTXO and not-in-basket are untouched', async () => {
    for (const storage of storages) {
      ...
      // services.getUtxoStatus: stale → false, live → true. External outpoint
      ...
      expect(staleAfter.cacheUpdatedAt).toBeInstanceOf(Date)
  ...
  test('checks basket-owned inputs concurrently before serial stale writes', async () => {
    for (const storage of storages) {
      ...
      for (const txid of fundingTxids) {
        const inserted = await _tu.insertTestTransaction(storage, user, false, {
          status: 'completed',
          txid
        })
        user = inserted.user
        const output = await _tu.insertTestOutput(storage, inserted.tx, 0, 1000, undefined, false, {
          txid,
          lockingScript: lockingScript.toBinary(),
          spendable: true,
          spentBy: undefined
      ...
      failedTx.addOutput({ lockingScript, satoshis: 2500 })
      const failedTxid = failedTx.id('hex')
      ...
      const ar = makeAggregateResult(new EntityProvenTxReq(refreshed))
      const staleFundingTxids = new Set([fundingTxids[0], fundingTxids[2]])
      let active = 0
      let maxActive = 0

      const services = mockServices(async (output: any) => {
        const txidHex = typeof output.txid === 'string'
          ? output.txid
          : Buffer.from(output.txid).toString('hex')
        active++
        ...
        try {
          ...
          if (txidHex === liveFundingTxid) return true
          if (staleFundingTxids.has(txidHex)) return false
          throw new Error(`unexpected isUtxo lookup for ${txidHex}`)
        } finally {
          active--
        }
      ...
      const firstAfter = verifyOne(await storage.findOutputs({ partial: { outputId: outputIds[0] } }))
      const liveAfter = verifyOne(await storage.findOutputs({ partial: { outputId: outputIds[1] } }))
      const thirdAfter = verifyOne(await storage.findOutputs({ partial: { outputId: outputIds[2] } }))
      expect(firstAfter.spendable).toBe(false)
      expect(firstAfter.cacheUpdatedAt).toBeInstanceOf(Date)
      expect(liveAfter.spendable).toBe(true)
      expect(thirdAfter.spendable).toBe(false)
      expect(thirdAfter.cacheUpdatedAt).toBeInstanceOf(Date)
    }
  })
  ...
  test('broadcaster-agnostic: applies the same decision under ar.status==="invalidTx" (WoC/Bitails missing-inputs path)', async () => {
    for (const storage of storages) {
      ...
      // decision is based on fresh services.getUtxoStatus, not on aggregate-status
      // classification.
      const ar = makeAggregateResult(seed.failedReq)
      ar.status = 'invalidTx'
  ...
  describe('integration: updateReqsFromAggregateResults wiring', () => {
    async function seedForIntegration (storage: StorageProvider): Promise<{
      ...
      return {
        ...
        failedTxId: failedRecord.transactionId!,
        failedTxid,
        consumedOutpoint: `${fundingTxid}.0`,
        consumedOutputId: consumedOutput.outputId!,
        txids: [failedTxid]
      }
    }
    ...
    function makeApbrs (
      failedReq: EntityProvenTxReq,
      aggStatus: 'success' | 'doubleSpend' | 'invalidTx'
    ...
    function mockServicesForIntegration (
      isUtxoFn: () => boolean | Promise<boolean>,
      eventBus?: { emitUtxoInvalidation: (event: { outpoints: string[] }) => void }
    ): sdk.WalletServices {
      return {
        eventBus,
        getUtxoStatus: async (_output: string, _format: any, _outpoint?: string, useNext?: boolean) => {
          expect(useNext).toBe(true)
          return { name: 'mock', status: 'success' as const, isUtxo: await isUtxoFn(), details: [] }
        },
    ...
    test('success aggregate invalidates consumed input and created output cache entries', async () => {
      for (const storage of storages) {
        const seed = await seedForIntegration(storage)
        const apbrs = makeApbrs(seed.failedReq, 'success')
        const r = makeResult(seed.failedReq)
        const eventBus = { emitUtxoInvalidation: jest.fn() }
        const services = mockServicesForIntegration(() => true, eventBus)

        await updateReqsFromAggregateResults(seed.txids, r, apbrs, storage as StorageKnex, services, undefined)

        expect(eventBus.emitUtxoInvalidation).toHaveBeenCalledWith({
          outpoints: [seed.consumedOutpoint, `${seed.failedTxid}.0`]
        })
      }
    })
  ...
  test('skips inputs not in the user\'s basket (does not touch other users\' UTXOs)', async () => {
    for (const storage of storages) {
      ...
      const ar = makeAggregateResult(seed.failedReq)
      let utxoStatusCalled = false
      const services = mockServices(() => {
        utxoStatusCalled = true
        return false
      })
      ...
      expect(result.staleConfirmed).toBe(0)
      expect(utxoStatusCalled).toBe(false)
