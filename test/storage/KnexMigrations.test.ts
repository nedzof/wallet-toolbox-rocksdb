...
describe('KnexMigrations tests', () => {
  ...
  test('4 adds output cacheUpdatedAt metadata column', async () => {
    const localSQLiteFile = await _tu.newTmpFile('migrateoutputcachetest.sqlite', false, false, false)
    const knex = _tu.createLocalSQLite(localSQLiteFile)
    ...
    try {
      ...
      const source = new KnexMigrations('test', 'output cache migration test', '1'.repeat(64), 1000)
      const migration = await source.getMigration('2026-05-15-002 add output cache updated at')
      await migration.up(knex)

      expect(await knex.schema.hasColumn('outputs', 'cacheUpdatedAt')).toBe(true)

      await migration.down?.(knex)

      expect(await knex.schema.hasColumn('outputs', 'cacheUpdatedAt')).toBe(false)
    } finally {
      await knex.destroy()
    }
  })

  test('5 adds and backfills output scriptHash index column', async () => {
    const localSQLiteFile = await _tu.newTmpFile('migrateoutputscriptindex.sqlite', false, false, false)
    const knex = _tu.createLocalSQLite(localSQLiteFile)
    const lockingScript = [0x76, 0xa9, 0x14, ...Array(20).fill(1), 0x88, 0xac]
    ...
    try {
      ...
      const source = new KnexMigrations('test', 'output script hash migration test', '1'.repeat(64), 1000)
      const migration = await source.getMigration('2026-05-15-001 add output script hash indexes')
      await migration.up(knex)

      expect(await knex.schema.hasColumn('outputs', 'scriptHash')).toBe(true)
      const rows = await knex('outputs').select('outputId', 'scriptHash').orderBy('outputId')

      expect(rows[0].scriptHash).toBe(hashOutputLockingScript(lockingScript))
      expect(rows[1].scriptHash).toBeNull()

      await migration.down?.(knex)

      expect(await knex.schema.hasColumn('outputs', 'scriptHash')).toBe(false)
    } finally {
      await knex.destroy()
    }
  })
