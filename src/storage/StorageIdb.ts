...
export class StorageIdb extends StorageProvider implements WalletStorageProvider {
  ...
  async insertOutput (output: TableOutput, trx?: TrxToken): Promise<number> {
    ...
    const e = await this.validateEntityForInsert(output, trx, outputDateFields)
  ...
  async updateIdb<T>(
    ...
    const u = this.validatePartialForUpdate(update, dateFields, booleanFields)
  ...
  async updateOutput (id: number, update: Partial<TableOutput>, trx?: TrxToken): Promise<number> {
    ...
    return await this.updateIdb(id, update, 'outputId', 'outputs', trx, outputDateFields)
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (
      ...
      partial?.basketId !== undefined &&
      partial?.spendable !== undefined &&
      partial?.scriptHash !== undefined
    ) {
      return store.index('userId_basketId_spendable_scriptHash').openCursor(
        [partial.userId, partial.basketId, partial.spendable, partial.scriptHash],
        direction
      )
    }
    if (partial?.scriptHash !== undefined) return store.index('scriptHash').openCursor(partial.scriptHash, direction)
    if (partial?.userId !== undefined) {
  ...
  async findOutputs (args: FindOutputsArgs, tagIds?: number[], isQueryModeAll?: boolean): Promise<TableOutput[]> {
    ...
    await this.filterOutputs(
      args,
      r => {
        results.push(this.validateEntity(r, outputDateFields))
      },
      tagIds,
      isQueryModeAll
    )
  ...
  private applyBooleanFields (entity: Record<string, unknown>, booleanFields?: string[]): void {
    ...
    for (const df of booleanFields) {
      if (entity[df] !== undefined) entity[df] = !(entity[df] == null)
    }
  }
