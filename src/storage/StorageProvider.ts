...
const STORAGE_PROVIDER_BULK_READ_CONCURRENCY = 50
...
export abstract class StorageProvider extends StorageReaderWriter implements WalletStorageProvider {
  ...
  async findOutputsByIds (outputIds: number[], trx?: TrxToken): Promise<Record<number, TableOutput>> {
    ...
    const outputs = await Promise.all(outputIds.map(async outputId => await limit(
      async () => await this.findOutputs({ partial: { outputId }, trx })
    )))
    for (const rows of outputs) {
      const o = verifyOneOrNone(rows)
  ...
  async findOutputsByOutpoints (
    ...
    const limit = pLimit(STORAGE_PROVIDER_BULK_READ_CONCURRENCY)
    const outputs = await Promise.all(outpoints.map(async ({ txid, vout }) => await limit(
      async () => await this.findOutputs({ partial: { userId, txid, vout }, trx })
    )))
    for (const rows of outputs) {
      const o = verifyOneOrNone(rows)
  ...
  async findOrInsertOutputBasketsBulk (
    ...
    const limit = pLimit(STORAGE_PROVIDER_BULK_READ_CONCURRENCY)
    const baskets = await Promise.all(uniqueNames.map(async name => await limit(
      async () => await this.findOrInsertOutputBasket(userId, name, trx)
    )))
    for (const basket of baskets) byName[basket.name] = basket
    return byName
  }
  ...
  async findOrInsertOutputTagsBulk (
    ...
    const limit = pLimit(STORAGE_PROVIDER_BULK_READ_CONCURRENCY)
    const outputTags = await Promise.all(uniqueTags.map(async tag => await limit(
      async () => await this.findOrInsertOutputTag(userId, tag, trx)
    )))
    for (const tag of outputTags) byTag[tag.tag] = tag
    return byTag
  }
  ...
  async getReqsAndBeefToShareWithWorld (
    ...
    const limit = pLimit(STORAGE_PROVIDER_BULK_READ_CONCURRENCY)
    r.details = await Promise.all(txids.map(async txid => await limit(async () => await this.getReqToShareDetail(txid, trx))))

    for (const d of r.details) {
      if (d.status !== 'readyToSend') continue
      try {
  ...
  private async getReqToShareDetail (txid: string, trx?: TrxToken): Promise<GetReqsAndBeefDetail> {
  ...
  private async checkOutputIsUtxo (
    o: TableOutput,
    services: { hashOutputScript: (s: string) => string, getUtxoStatus: (hash: string, fmt: undefined, outpoint: string, useNext?: boolean) => Promise<{ isUtxo?: boolean }> }
  ): Promise<boolean> {
    ...
    const r = await services.getUtxoStatus(hash, undefined, `${o.txid ?? ''}.${o.vout ?? ''}`, true)
