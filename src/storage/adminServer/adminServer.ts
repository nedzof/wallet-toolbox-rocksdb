...
export class AdminServer {
  ...
  private setupRoutes (): void {
    ...
    this.app.get('/metrics', async (_req: any, res: any) => {
      const services = this.context.daemon.setup?.services
      if (services == null) {
        res.status(503).type('text/plain').send('Services metrics are not available.')
        return
      }
      res.type(services.metrics.contentType).send(await services.metrics.metrics())
    })
