...
function isValidListOutputsArgs (args: ListOutputsArgs | Validation.ValidListOutputsArgs): args is Validation.ValidListOutputsArgs {
  const validated = args as Partial<Validation.ValidListOutputsArgs>
  return (
    typeof validated.includeLockingScripts === 'boolean' &&
    typeof validated.includeTransactions === 'boolean' &&
    Array.isArray(validated.tags) &&
    typeof validated.limit === 'number' &&
    typeof validated.offset === 'number'
  )
}
...
export class WalletStorageManager implements sdk.WalletStorage {
  ...
  async listActions (args: ListActionsArgs): Promise<ListActionsResult> {
    const vargs = Validation.validateListActionsArgs(args)
    const auth = await this.getAuth()
  ...
  async listOutputs (args: ListOutputsArgs | Validation.ValidListOutputsArgs): Promise<ListOutputsResult> {
    const vargs = isValidListOutputsArgs(args) ? args : Validation.validateListOutputsArgs(args)
    const auth = await this.getAuth()
    return await this.runAsReader(async reader => {
