...
function matchesOutputPartialStrings (r: TableOutput, partial: Partial<TableOutput>): boolean {
  return (
    ...
    eqNullable(partial.scriptHash, r.scriptHash)
  )
}
...
export function upgradeOutputs (db: IDBPDatabase<StorageIdbSchema>): void {
  ...
  store.createIndex('scriptHash', 'scriptHash')
  store.createIndex('userId_basketId_spendable_scriptHash', ['userId', 'basketId', 'spendable', 'scriptHash'])
