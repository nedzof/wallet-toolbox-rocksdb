...
export class StorageServer {
  ...
  private readonly rateLimit?: WalletStorageServerRateLimitOptions
  ...
  private setupRoutes (): void {
    ...
    this.app.use(this.rateLimitAuthenticatedRequest.bind(this))
    if (this.monetize) {
      this.app.use(
  ...
  private rateLimitAuthenticatedRequest (req: Request, res: Response, next: express.NextFunction): void {
    ...
    const allowed = this.rateLimiter.checkLimit(identityKey, this.rateLimit.maxRequests, this.rateLimit.windowMs)
    if (!allowed) {
      res.status(429).json({
        error: {
