...
function tallyTxidResults (
  ...
  for (const pbr of pbrs) {
    ...
    ar.providerAttempts.push(formatProviderAttemptForHistory(pbr.name, tr))
...
export async function updateReqsFromAggregateResults (
  ...
  for (const txid of txids) {
    ...
    const note: ReqHistoryNote = {
      ...
      statusErrorCount,
      serviceErrorCount,
      providerCount: ar.providerAttempts.length,
      providerAttempts: ar.providerAttempts.join(';')
    }
    ...
    if (newTxStatus === 'unproven') emitBroadcastOutpointInvalidation(req, services)
...
export async function markStaleInputsAsSpent (
  ...
  const checks: CheckResult[] = await Promise.all(
    outpoints.map(async (outpoint): Promise<CheckResult> => await limit(async () => {
      ...
      try {
        isStillUtxo = await checkOutputIsFreshUtxo(localOutput, outpoint, services)
      } catch {
  ...
  for (const c of checks) {
    ...
    if (c.kind === 'stale') {
      ...
      await storage.updateOutput(
        ...
        { spendable: false, cacheUpdatedAt: new Date() },
        trx
      )
  ...
  if (result.staleConfirmed > 0) {
    services.eventBus?.emitUtxoInvalidation({
      outpoints: result.staleOutpoints
    })
    logger?.log(
...
function emitBroadcastOutpointInvalidation (
  req: EntityProvenTxReq,
  services?: sdk.WalletServices
): void {
  if (services?.eventBus == null) return
  try {
    const outpoints = collectReqOutpointsForInvalidation(req)
    if (outpoints.length > 0) services.eventBus.emitUtxoInvalidation({ outpoints })
  } catch {
    // Cache invalidation is best-effort. Storage status changes above remain
    // authoritative and must not be rolled back because a malformed historical
    // rawTx could not be parsed for cache hygiene.
  }
}

function collectReqOutpointsForInvalidation (req: EntityProvenTxReq): string[] {
  const tx = Transaction.fromBinary(req.rawTx)
  const outpoints = new Set<string>()
  for (const input of tx.inputs) {
    if (input.sourceTXID != null && input.sourceOutputIndex != null) {
      outpoints.add(`${input.sourceTXID}.${input.sourceOutputIndex}`)
    }
  }
  const txid = req.txid || tx.id('hex')
...
async function checkOutputIsFreshUtxo (
  output: TableOutput,
  outpoint: { txid: string, vout: number },
  services: sdk.WalletServices
): Promise<boolean> {
  const scriptHash = services.hashOutputScript(asString(output.lockingScript!))
  const status = await services.getUtxoStatus(scriptHash, undefined, `${outpoint.txid}.${outpoint.vout}`, true)
  if (status.status !== 'success') throw status.error ?? new Error('UTXO_STATUS_UNAVAILABLE')
  return status.isUtxo === true
}
...
export function summarizePostBeefProviderAttemptsForTxid (
  txid: string,
  pbrs: sdk.PostBeefResult[]
): string[] {
  ...
  for (const pbr of pbrs) {
    ...
    if (tr != null) attempts.push(formatProviderAttemptForHistory(pbr.name, tr))
  }
  return attempts
}

function formatProviderAttemptForHistory (providerName: string, tr: sdk.PostTxResultForTxid): string {
  return `${encodeURIComponent(providerName)}=${tr.status}/${classifyProviderAttempt(tr)}`
}

function classifyProviderAttempt (tr: sdk.PostTxResultForTxid): string {
  if (tr.status === 'success') return tr.alreadyKnown === true ? 'alreadyKnown' : 'accepted'
  if (tr.doubleSpend === true) return 'doubleSpend'
  if (tr.serviceError === true) return 'serviceError'
  return 'rejected'
}
