...
export class KnexMigrations implements MigrationSource<string> {
  ...
  setupMigrations (
    ...
    migrations['2026-05-15-001 add output script hash indexes'] = {
      async up (knex) {
        await knex.schema.alterTable('outputs', table => {
          table.string('scriptHash', 64).nullable()
          table.index('scriptHash', 'idx_outputs_script_hash')
          table.index(['userId', 'basketId', 'spendable', 'scriptHash'], 'idx_outputs_user_basket_spendable_script_hash')
        })
        const rows = await knex('outputs')
          .whereNotNull('lockingScript')
          .whereNull('scriptHash')
          .select('outputId', 'lockingScript')
        for (const row of rows) {
          const lockingScript = row.lockingScript == null ? undefined : Array.from(row.lockingScript as Buffer)
          if (lockingScript == null || lockingScript.length === 0) continue
          await knex('outputs')
            .where({ outputId: row.outputId })
            .update({ scriptHash: hashOutputLockingScript(lockingScript) })
        }
      },
      async down (knex) {
        await knex.schema.alterTable('outputs', table => {
          table.dropIndex(['userId', 'basketId', 'spendable', 'scriptHash'], 'idx_outputs_user_basket_spendable_script_hash')
          table.dropIndex('scriptHash', 'idx_outputs_script_hash')
          table.dropColumn('scriptHash')
        })
      }
    }

    migrations['2026-05-15-002 add output cache updated at'] = {
      async up (knex) {
        await knex.schema.alterTable('outputs', table => {
          table.timestamp('cacheUpdatedAt', { precision: 3 }).nullable()
        })
      },
      async down (knex) {
        await knex.schema.alterTable('outputs', table => {
          table.dropColumn('cacheUpdatedAt')
        })
      }
    }
