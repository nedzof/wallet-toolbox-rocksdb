...
export interface BuildBeefHttpClient {
  request: (url: string, options: { method?: string, signal?: AbortSignal }) => Promise<{
    ok: boolean
    status: number
    statusText: string
    data: unknown
...
export function resolveAutoSigned (car: CreateActionResult, txid: string, vout: number): string {
  if (car.txid == null || car.txid === '' || !/^[0-9a-f]{64}$/i.test(car.txid)) {
...
export async function signAndComplete (
  ...
  const unlockingScript = unsignedTx.inputs[inputIndex].unlockingScript
  if (unlockingScript == null) throw new Error('Transaction signing did not produce an unlocking script')
  const sar = await wallet.signAction({ reference: st.reference, spends: { [inputIndex]: { unlockingScript: unlockingScript.toHex() } } })
  if (sar.txid == null || sar.txid === '' || !/^[0-9a-f]{64}$/i.test(sar.txid)) throw new Error('signAction returned no valid txid')
  return sar.txid
}
...
export async function importSingleOutpoint (
  ...
  if (satoshis == null || satoshis <= 0) throw new Error(`Output ${outpoint} has no satoshis`)
 ...
 * @internal
 */
export async function buildBeefForOutpoints (
  outpoints: string[],
  optionsOrMaxDepth: number | BuildBeefForOutpointsOptions = {}
): Promise<BEEF> {
  const options: BuildBeefForOutpointsOptions = typeof optionsOrMaxDepth === 'number'
    ? { maxDepth: optionsOrMaxDepth }
    : optionsOrMaxDepth
  const maxDepth = options.maxDepth ?? 10
  const requestTimeoutMs = options.requestTimeoutMs ?? 8000
  const httpClient = options.httpClient ?? createDefaultBuildBeefHttpClient()
  const closeClient = options.httpClient == null
  const beef = new Beef()
  ...
  async function fetchRawTx (txid: string): Promise<string | null> {
    ...
    for (const url of providers) {
      try {
        const response = await withTimeout(requestTimeoutMs, async signal => {
          return await httpClient.request(url, { method: 'GET', signal })
        })
        if (response.ok) return String(response.data).trim()
      } catch {
        /* try next */
      }
    }
  ...
  async function fetchMerklePath (txid: string): Promise<MerklePath | null> {
    try {
      const url = `https://ordinals.gorillapool.io/api/tx/${txid}/proof`
      const buf = await withTimeout(requestTimeoutMs, async signal => await download(httpClient, url, signal))
  ...
  async function addTxToBeef (txid: string, depth: number): Promise<void> {
    ...
    if (rawHex == null || rawHex === '') throw new Error(`Failed to fetch raw transaction ${txid} from any provider`)
    ...
    if (merklePath != null) {
      ...
      for (const input of tx.inputs) {
        if (input.sourceTXID != null && input.sourceTXID !== '') {
  ...
  try {
    ...
    if (closeClient && httpClient.close != null) await httpClient.close()
  }
...
async function download (httpClient: BuildBeefHttpClient, url: string, signal: AbortSignal): Promise<Uint8Array> {
  if (httpClient.download != null) return await httpClient.download(url, { signal })
  const response = await httpClient.request(url, { method: 'GET', signal })
  ...
  return new Uint8Array(Utils.toArray(String(response.data), 'base64'))
}

function createDefaultBuildBeefHttpClient (): BuildBeefHttpClient {
  const client = createUndiciHttpClient()
  return {
    request: async (url, options) => await client.request(url, options),
    download: async (url, options) => await client.download(url, options),
