...
const UTXO_STATUS_CONCURRENCY = 50
/**
  ...
  async unfailReq (req: EntityProvenTxReq, indent: number): Promise<string> {
    ...
    for (const id of txIds) {
      ...
      await this.storage.runAsStorageProvider(async sp => {
        ...
        const checkLimit = pLimit(UTXO_STATUS_CONCURRENCY)
        ...
        for (const o of outputs) {
          ...
          if (o.lockingScript == null) {
            ...
            checks.push(checkLimit(async () => {
              const isUtxo = await services.isUtxo(o, true)
              const line = isUtxo === o.spendable
                ? `output ${o.outputId} unchanged\n`
                : `output ${o.outputId} set to ${isUtxo ? 'spendable' : 'spent'}\n`
              return {
                output: o,
                isUtxo,
                log: ' '.repeat(indent + 2) + line
              }
            }))
          }
        }
        const checkedOutputs = await Promise.all(checks)
        for (const checked of checkedOutputs) {
          log += checked.log
          if (checked.isUtxo != null && checked.isUtxo !== checked.output.spendable) {
            await sp.updateOutput(checked.output.outputId, { spendable: checked.isUtxo, cacheUpdatedAt: new Date() })
          }
        }
      })
    }
