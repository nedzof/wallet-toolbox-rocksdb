...
export class TaskArcadeSSE extends WalletMonitorTask {
  ...
  private async processStatusEvent (event: ArcSSEEvent): Promise<string> {
    ...
    if (event.txStatus === 'BLOCK_MINED') {
      return log + this.handleBlockEvent(event)
    }
  ...
  private async fetchProofFromArcade (req: EntityProvenTxReq): Promise<string> {
    ...
    try {
      ...
      const httpClient = (this.monitor.services as Services).options?.httpClient
      if (httpClient == null) {
        log += '    No pooled HTTP client available for Arcade proof fetch\n'
        return log
      }
      const response = await httpClient.request<any>(`${arcUrl}/tx/${txid}`, { method: 'GET', headers: fetchHeaders })
      if (!response.ok) {
