...
function makeReq (provenTxReqId: number, txid: string, updatedAt: Date): TestReq {
  const now = new Date()
  return {
    provenTxReqId,
    created_at: now,
...
function makeMonitor (
  ...
  const findProvenTxReqs = jest.fn(async ({ paged }: any) => {
    const page = paged as { offset: number, limit: number }
    return reqs.slice(page.offset, page.offset + page.limit)
  })
  const findMonitorEvents = jest.fn().mockResolvedValue(monitorEvents)
  const runAsStorageProvider = jest.fn((fn: any) => fn({ updateProvenTxReq, findMonitorEvents }))
...
describe('TaskReviewDoubleSpends', () => {
  ...
  test('0 uses the normal cadence after a partial review chunk and stores checkpoint offset after unfails are removed', async () => {
    ...
    const task = new TaskReviewDoubleSpends(m.monitor, 0, 100, 60, 60)
    ...
    expect(m.monitor.services.getStatusForTxids).toHaveBeenCalledWith(['tx1', 'tx2'])
    expect(m.monitor.services.getStatusForTxids).toHaveBeenCalledTimes(1)
  ...
  test('1b uses the quick cadence after consuming a full review chunk', async () => {
    ...
    const m = makeMonitor({ tx1: 'success', tx2: 'unknown' }, reqs)
    const task = new TaskReviewDoubleSpends(m.monitor, 0, 2, 60, 60)
  ...
  test('2 resumes from the checkpoint offset and advances it', async () => {
    ...
    const task = new TaskReviewDoubleSpends(m.monitor, 0, 2, 60)
