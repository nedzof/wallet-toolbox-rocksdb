...
describe('TaskArcadeSSE', () => {
  ...
  describe('asyncSetup()', () => {
    ...
    test('asyncDestroy closes the SSE connection and clears pending events', async () => {
      const task = new TaskArcadeSSE(makeMonitor())
      await task.asyncSetup()
      const source = FakeEventSource.instances[0]
      FakeEventSource.instances[0].emit('status', {
        data: JSON.stringify({ txid: 'aaaa', txStatus: 'MINED', timestamp: '' })
      })
  ...
  describe('runTask()', () => {
    ...
    test('emits block events without ProvenTxReq lookup for BLOCK_MINED', async () => {
      const monitor = makeMonitor()
      const blocks: any[] = []
      monitor.eventBus.onBlockMined((event: any) => blocks.push(event))
      const task = new TaskArcadeSSE(monitor)
      await task.asyncSetup()
      FakeEventSource.instances[0].emit('status', {
        data: JSON.stringify({
