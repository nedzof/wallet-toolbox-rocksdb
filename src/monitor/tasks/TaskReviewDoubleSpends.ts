...
export class TaskReviewDoubleSpends extends WalletMonitorTask {
  ...
  async getLastReviewedCheckpoint (): Promise<{ resumeOffset: number, expectedProvenTxReqId?: number } | undefined> {
    ...
    for (const event of events) {
      if (event.details == null || event.details === '') continue
      try {
  ...
  async runTask (): Promise<string> {
    ...
    const gsr = await this.monitor.services.getStatusForTxids(reqs.map(req => req.txid))
    const statusByTxid = new Map(gsr.results.map(result => [result.txid, result.status]))

    for (const req of reqs) {
      const status = statusByTxid.get(req.txid)
      reviewed.push(req)
      if (status === 'unknown') {
        ...
        log += `unfail ${req.provenTxReqId} ${req.txid} status:${status ?? 'undefined'}\n`
      }
    }
    ...
    const sourceOffset = reqs.sourceOffset ?? 0
    return JSON.stringify({
      ...
      resumeOffset: lastRetainedDoubleSpendIndex >= 0 ? sourceOffset + retainedDoubleSpendCount - 1 : undefined,
