...
export class TaskSendWaiting extends WalletMonitorTask {
   ...
   * @param processConcurrency Maximum number of independent req/batch broadcasts to process concurrently.
   */
  constructor (
    monitor: Monitor,
    public triggerMsecs = Monitor.oneSecond * 1,
    public agedMsecs = Monitor.oneSecond * 1,
    public sendingMsecs = Monitor.oneMinute * 5,
    public triggerQuickMsecs = Monitor.oneSecond * 1,
    public chunkLimit = 500,
    public processConcurrency = 100
  ) {
  ...
  trigger (nowMsecsSinceEpoch: number): { run: boolean } {
    this.includeSending =
      this.lastSendingRunMsecsSinceEpoch == null ||
      nowMsecsSinceEpoch > this.lastSendingRunMsecsSinceEpoch + this.sendingMsecs
  ...
  private async expandBatches (reqs: TableProvenTxReq[], status: ProvenTxReqStatus[]): Promise<TableProvenTxReq[]> {
    ...
    const batchNames = Array.from(new Set(reqs
      .map(req => req.batch)
      .filter((batch): batch is string => batch != null && batch !== '')
    ))
    const batchReqsByName = new Map<string, TableProvenTxReq[]>()
    const limit = pLimit(Math.max(1, Math.min(this.processConcurrency, batchNames.length || 1)))
    await Promise.all(batchNames.map(async batch => await limit(async () => {
      batchReqsByName.set(batch, await this.storage.findProvenTxReqs({
        partial: { batch },
        status
      }))
    })))
    ...
    for (const req of reqs) {
      ...
      if (req.batch == null || req.batch === '' || seenBatches.has(req.batch)) {
      ...
      const batchReqs = batchReqsByName.get(req.batch) ?? []
  ...
  async processUnsent (reqApis: TableProvenTxReq[], indent = 0): Promise<string> {
    ...
    const workItems: SendWaitingWorkItem[] = []
    ...
    for (const reqApi of reqApis) {
      ...
      workItems.push({ reqApi, req, reqs })
    }

    const queue = new PQueue({ concurrency: Math.max(1, this.processConcurrency) })
    const prioritizedWorkItems = [...workItems].sort((a, b) => (b.reqApi.attempts ?? 0) - (a.reqApi.attempts ?? 0))
    const metrics = this.monitor.services?.metrics
    metrics?.setSendWaitingQueue(prioritizedWorkItems.length, 0)
    await Promise.all(
      prioritizedWorkItems.map(async item => await queue.add(
        async () => {
          metrics?.setSendWaitingQueue(queue.size, queue.pending)
          try {
            await this.processWorkItem(item, logs)
          } finally {
            metrics?.setSendWaitingQueue(queue.size, queue.pending)
          }
        },
        { priority: item.reqApi.attempts ?? 0 }
      ))
    )
    metrics?.setSendWaitingQueue(0, 0)
  ...
  private async processWorkItem (
    ...
    if (this.monitor.onTransactionBroadcasted != null) {
      const rar = await this.storage.runAsStorageProvider(async sp => {
        const ars: SendWithResult[] = [{ txid: item.req.txid, status: 'sending' }]
