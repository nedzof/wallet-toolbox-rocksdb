...
export class WalletError extends Error implements WalletErrorObject {
  ...
  static fromUnknown (err: unknown): WalletError {
    ...
    if (err !== null && typeof err === 'object') {
      ...
      if (fields.name === 'Error' || fields.name === 'FetchError') name = asErrorCode(fields.code) ?? asErrorCode(fields.status) ?? 'WERR_UNKNOWN'
      else name = asErrorCode(fields.name) ?? asErrorCode(fields.code) ?? asErrorCode(fields.status) ?? 'WERR_UNKNOWN'
      if (typeof name !== 'string') name = 'WERR_UNKNOWN'

      message = asErrorMessage(fields.message) ?? asErrorMessage(fields.description) ?? ''
      if (typeof message !== 'string') message = 'WERR_UNKNOWN'

      if (typeof fields.stack === 'string') stack = fields.stack

      if (typeof fields.sql === 'string') details.sql = fields.sql
      if (typeof fields.sqlMessage === 'string') details.sqlMessage = fields.sqlMessage
    }
