...
export class WalletPermissionsManager implements WalletInterface {
  ...
  > = new Map()
  private readonly manifestHttpClient: Pick<UndiciHttpClient, 'request'> = createUndiciHttpClient()
  ...
  private async fetchManifestPermissions (originator: string): Promise<{
    ...
    const fetchPromise = (async (): Promise<{
      ...
      try {
        ...
        const response = await this.manifestHttpClient.request<any>(`${proto}://${originator}/manifest.json`, { method: 'GET' })
        if (response.ok) {
