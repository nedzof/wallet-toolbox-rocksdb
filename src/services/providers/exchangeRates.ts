...
export async function updateChaintracksFiatExchangeRates (
  ...
  const httpClient = options.httpClient ?? createUndiciHttpClient()
  const r = await httpClient.request<any>(url, { method: 'GET' })
...
export async function updateExchangeratesapi (
  ...
  const iorates = await getExchangeRatesIo(options.exchangeratesapiKey, unique, options.httpClient)
...
export async function getExchangeRatesIo (
  key: string,
  symbols?: string[],
  httpClient: HttpClient = createUndiciHttpClient()
  ...
  const r = await httpClient.request<ExchangeRatesIoApi>(url, { method: 'GET' })
