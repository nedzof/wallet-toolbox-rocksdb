...
import type { FiatCurrencyCode, PostBeefMode } from '../sdk/WalletServices.interfaces'
...
import { UtxoCacheManager } from '../cache/UtxoCacheManager'
import { BlockHeaderCache } from '../cache/BlockHeaderCache'
import { ScriptHashCache } from '../cache/ScriptHashCache'
import { EventBus } from '../events/EventBus'
import { WalletToolboxMetrics } from '../metrics/WalletToolboxMetrics'
import { createUndiciHttpClient, isWalletToolboxOwnedHttpClient } from '../http/UndiciHttpClient'

interface ClosableHttpClient {
...
export class Services implements WalletServices {
  ...
  private readonly postBeefQueue: PQueue
  private readonly activePostBeef = new Set<Promise<PostBeefResult[]>>()
  private readonly ownsHttpClient: boolean
  private readonly ownsUtxoCache: boolean
  private readonly ownsBlockHeaderCache: boolean
  private readonly ownsScriptHashCache: boolean
  ...
  constructor (optionsOrChain: Chain | WalletServicesOptions) {
    ...
    this.ownsHttpClient = typeof optionsOrChain === 'string' ||
      this.options.httpClient == null ||
      isWalletToolboxOwnedHttpClient(this.options.httpClient)
    ...
    this.postBeefUntilSuccessSoftTimeoutMaxMs = this.options.postBeefSoftTimeoutMaxMs ?? this.postBeefUntilSuccessSoftTimeoutMaxMs
    this.eventBus = this.options.eventBus ?? new EventBus()
    this.metrics = this.options.metrics ?? new WalletToolboxMetrics()
    this.utxoCache = this.options.utxoCache ?? new UtxoCacheManager({
      max: this.options.utxoStatusCacheMaxEntries,
      ttlMs: this.options.utxoStatusCacheTtlMs,
      events: this.eventBus,
      metrics: this.metrics
    })
    this.blockHeaderCache = this.options.blockHeaderCache ?? new BlockHeaderCache({
      maxEntries: this.options.blockHeaderCacheMaxEntries,
      ttlMs: this.options.blockHeaderCacheTtlMs,
      events: this.eventBus,
      metrics: this.metrics
    })
    this.scriptHashCache = this.options.scriptHashCache ?? new ScriptHashCache({
      max: this.options.scriptHashCacheMaxEntries,
      ttlMs: this.options.scriptHashCacheTtlMs
    })
    this.postBeefQueue = new PQueue({ concurrency: this.options.postBeefQueueConcurrency ?? 100 })

    const httpClient = this.options.httpClient
    const arcConfig = (httpClient != null && this.options.arcConfig.httpClient == null)
      ? { ...this.options.arcConfig, httpClient }
      : this.options.arcConfig
    const arcGorillaPoolConfig = (httpClient != null && this.options.arcGorillaPoolConfig?.httpClient == null)
      ? { ...this.options.arcGorillaPoolConfig, httpClient }
      : this.options.arcGorillaPoolConfig

    this.whatsonchain = new WhatsOnChain(this.chain, { apiKey: this.options.whatsOnChainApiKey, httpClient }, this)

    this.arcTaal = new ARC(this.options.arcUrl, arcConfig, 'arcTaal')
    if (this.options.arcGorillaPoolUrl) {
      this.arcGorillaPool = new ARC(this.options.arcGorillaPoolUrl, arcGorillaPoolConfig, 'arcGorillaPool')
    }

    const hasBitails = this.chain === 'main' || this.chain === 'test'

    if (hasBitails) {
      this.bitails = new Bitails(this.chain, { apiKey: this.options.bitailsApiKey, httpClient })
    }
  ...
  async close (): Promise<void> {
    ...
    this.closed = true
    await Promise.allSettled(Array.from(this.activePostBeef))
    await this.postBeefQueue.onIdle()
    if (this.ownsUtxoCache) this.utxoCache.close()
    if (this.ownsBlockHeaderCache) this.blockHeaderCache.close()
    if (this.ownsScriptHashCache) this.scriptHashCache.close()
    if (this.ownsHttpClient) {
      const close = (this.options.httpClient as ClosableHttpClient | undefined)?.close
      if (typeof close === 'function') await close.call(this.options.httpClient)
    }
  }
  ...
  private async getStatusForTxidsBatched (
    ...
    const limit = pLimit(Services.getStatusForTxidsBatchConcurrency)
    const batches: string[][] = []

    for (let i = 0; i < txids.length; i += Services.getStatusForTxidsBatchLimit) {
      batches.push(txids.slice(i, i + Services.getStatusForTxidsBatchLimit))
    }

    const batchResults = await Promise.all(
      batches.map(async batch => await limit(async () => await stc.service(batch)))
    )
  ...
  hashOutputScript (script: string): string {
    return this.scriptHashCache.getOrCompute(script, () => Utils.toHex(sha256Hash(Utils.toArray(script, 'hex'))))
  }
  ...
  async getUtxoStatus (
    ...
    if (outpoint != null && !useNext) {
      return await this.utxoCache.getOrLoad(
        { output, outputFormat, outpoint },
        async () => await this.getUtxoStatusFromProviders(output, outputFormat, outpoint, useNext, logger)
      )
    }

    return await this.getUtxoStatusFromProviders(output, outputFormat, outpoint, useNext, logger)
  }

  private async getUtxoStatusFromProviders (
    output: string,
    outputFormat?: GetUtxoStatusOutputFormat,
    outpoint?: string,
    useNext?: boolean,
  ...
  async postBeef (beef: Beef, txids: string[], logger?: WalletLoggerInterface): Promise<PostBeefResult[]> {
    if (this.closed) throw new WERR_INVALID_OPERATION('Services are closed.')
    const operation = this.runPostBeef(beef, txids, logger)
    this.activePostBeef.add(operation)
    try {
      return await operation
    } finally {
      this.activePostBeef.delete(operation)
    }
  }

  private async runPostBeef (beef: Beef, txids: string[], logger?: WalletLoggerInterface): Promise<PostBeefResult[]> {
    ...
    const postBeefQueue = this.postBeefQueue
    ...
    switch (this.postBeefMode) {
      ...
      case 'PromiseAll':
        rs = await Promise.all(
          stcs.map(async stc => {
            const r = await this.postBeefQueue.add(
              async () => await callService(stc, softTimeoutMs),
              { priority: 0 }
            )
            return r
          })
        )
        this.metrics.setPostBeefQueue(this.postBeefQueue.size, this.postBeefQueue.pending)
        break
    }
    logger?.groupEnd()
    return rs
    ...
    async function callService (stc: ServiceToCall<PostBeefService>, timeoutMs?: number) {
      ...
      try {
        ...
        r = makeServiceExceptionResult(stc.providerName, txids, WalletError.fromUnknown(error_))
      }
      ...
      metrics.recordPostBeefProvider(stc.providerName, r.status, Date.now() - start)
      metrics.setPostBeefQueue(postBeefQueue.size, postBeefQueue.pending)
      return r
    }

    function makeServiceExceptionResult (providerName: string, txids: string[], error: WalletError): PostBeefResult {
      return {
        name: providerName,
        status: 'error',
        error,
        ...
        notes: [{ when: new Date().toISOString(), what: 'postBeefServiceException', providerName, code: error.code, message: error.description }]
      }
    }
  ...
  async getHeaderForHeight (height: number): Promise<number[]> {
    const cached = this.blockHeaderCache.getByHeight(height)
    if (cached != null) return toBinaryBaseBlockHeader(cached)
  ...
  async hashToHeader (hash: string): Promise<BlockHeader> {
    const cached = this.blockHeaderCache.getByHash(hash)
