import { BlockHeadersService, HttpClient, Utils } from '@bsv/sdk'
...
export class BHServiceClient implements ChaintracksServiceClient {
  ...
  constructor (chain: Chain, url: string, apiKey: string, httpClient?: HttpClient) {
    ...
    this.apiKey = apiKey
    this.httpClient = httpClient ?? createUndiciHttpClient()
  }
  ...
  async getJsonOrUndefined<T>(path: string): Promise<T | undefined> {
    ...
    for (let retry = 0; retry < 3; retry++) {
      try {
        const r = await this.httpClient.request<T>(`${this.serviceUrl}${path}`, {
          method: 'GET',
